...
function apply_slot_data(slot_data)
	...
	for key, value in pairs(slot_data) do
		if key == "character" then
			Tracker:FindObjectForCode("opt_character").CurrentStage = value
			if value == 0 then
				Tracker:UiHint("ActivateTab", "Ventus")
			elseif value == 1 then
				Tracker:UiHint("ActivateTab", "Aqua")
			else
				Tracker:UiHint("ActivateTab", "Terra")
			end
		elseif key == "mirage_arena" then
			Tracker:FindObjectForCode("opt_mirage_arena").CurrentStage = bool_to_number(value)
		elseif key == "command_board" then
			Tracker:FindObjectForCode("opt_command_board").CurrentStage = bool_to_number(value)
		elseif key == "minigames" then
			Tracker:FindObjectForCode("opt_minigames").CurrentStage = bool_to_number(value)
		elseif key == "arena_medals" then
			Tracker:FindObjectForCode("opt_arena_medals").CurrentStage = bool_to_number(value)
		elseif key == "superbosses" then
			Tracker:FindObjectForCode("opt_superbosses").CurrentStage = bool_to_number(value)
		elseif key == "arena_global" then
			Tracker:FindObjectForCode("opt_arena_global").CurrentStage = bool_to_number(value)
		elseif key == "advanced_logic" then
			Tracker:FindObjectForCode("opt_advanced_logic").CurrentStage = bool_to_number(value)
		elseif key == "realm" then
			Tracker:FindObjectForCode("opt_realm").CurrentStage = bool_to_number(value)
		elseif key == "final_terranort" then
			Tracker:FindObjectForCode("opt_final_terranort").CurrentStage = bool_to_number(value)
		end
    end
end
